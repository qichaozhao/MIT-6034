...
if __name__ == "__main__":
    ...
    subset = {}
    for shortname in chosen:
        subset[shortname] = learners[shortname]

    describe_and_classify(dataset, subset)
    print "Boosting with our suite of orange classifiers:"
    print ("  accuracy: %.3f, brier: %.3f, auc: %.3f" %
           boosted_ensemble(dataset, subset, DATASET_STANDARDS[dataset], True))
...
classifiers_for_best_ensemble = ['svmp3',
                                 'svmr',
                                 'svms',
                                 'nb']
