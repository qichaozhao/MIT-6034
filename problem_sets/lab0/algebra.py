...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    # If they are sum/product the wrong way around, just call the function again with them switched
    ...
    if isinstance(expr1, Sum) and isinstance(expr2, Product):
        print "Sum * Product"
        print str(Sum([Product([expr2, term]).flatten() for term in expr1]).flatten())
        return Sum([Product([expr2, term]).flatten() for term in expr1]).flatten()

    if isinstance(expr1, Sum) and isinstance(expr2, Sum):
        print "Sum * Sum"
        print str(Sum([Product([term1, term2] for term1 in expr1).flatten() for term2 in expr2]).flatten())
        return Sum([Product([term1, term2] for term1 in expr1).flatten() for term2 in expr2]).flatten()
